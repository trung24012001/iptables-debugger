...
if r['src'] in addrs:
    ...
    print('Error: src and dst not valid')
    exit(1)
...
def parse_addr(addr):
    ...
    if addr.find('!') != -1:
        addr = addr.split('!')[1]
        return list(ipaddress.ip_network(u'0.0.0.0/0').address_exclude(ipaddress.ip_network(addr)))
...
def handle_prot(rprot, rsport, rdport, prot, rule):
